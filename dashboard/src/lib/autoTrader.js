...
const EXECUTION_LOGS_FILE = path.join(process.cwd(), 'trade_execution_logs.json');
const JOURNAL_FILE = path.join(process.cwd(), '..', 'trade_journal.json');
...
export async function runPostTriggerAIAnalysis(summary, alert, spotPrice, strategy) {
  try {
    ...
    if (!apiKey) {
      return {
        executed: false,
        summary: "Post-trigger analysis skipped (OPENAI_API_KEY not configured in environment).",
...
3. Risk management tip for this pre-approved ${strategy}
    ...
    const response = await openai.chat.completions.create({
      model: env.OPENAI_MODEL || process.env.OPENAI_MODEL || 'gpt-5-mini',
